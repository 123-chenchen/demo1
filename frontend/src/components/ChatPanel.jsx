...
export function ChatPanel({
  ...
  const headerTopic =
    suggestionTitle ||
    buildHeaderTopic(selectedDocuments, selectedDocument, text);
        ...
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            {!readySelectedDocuments.length &&
              ...
              selectedDocument && (
                <span
                  className={`inline-flex w-fit rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${statusClass(selectedDocument.status)}`}
                >
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
        ...
        {!showSuggestions &&
          ...
          !selectedDocuments.length && (
            <ChatEmptyState title={text.addSourcesTitle} />
          )}
...
function SourceScopeList({ documents, language }) {
  if (!documents.length) {
    return (
      <p className="mt-1 text-xs text-zinc-500">{"No selected sources"}</p>
    );
  }
      ...
      {documents.map((document) => (
        ...
        >
          ...
          <span className="max-w-[180px] truncate">
            {displayOriginalFileName(document)}
          </span>
          <span
            className={`h-1.5 w-1.5 rounded-full ${document.status === "processed" ? "bg-emerald-500" : "bg-amber-500"}`}
          />
        </span>
      ))}
    </div>
...
function buildHeaderTopic(selectedDocuments, selectedDocument, text) {
  if (selectedDocuments.length > 1) {
    const fileNames = selectedDocuments
      .map(displayOriginalFileName)
      .filter(Boolean);
    return fileNames.length
      ? fileNames.join(" / ")
      : `${text.selectedFiles} (${selectedDocuments.length})`;
  }
  return (
    displayOriginalFileName(selectedDocuments[0] || selectedDocument) ||
...
function SuggestedQuestions({
  ...
  const emptyText = "No content-based suggestions are available yet.";
...
function ChatMessage({ message, language, onSourceSelect, selectedSource }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && (
        <div
          className={`mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${message.isError ? "bg-rose-100 text-rose-700" : "bg-teal-100 text-teal-700"}`}
        >
          <Bot size={18} />
        </div>
      )}
      ...
      <div
        className={`max-w-[82%] rounded-lg px-4 py-3 text-sm leading-6 ${bubbleClass}`}
      >
...
function formatPageLabel(source, language) {
  ...
  const text = { page: "page", pages: "pages" };

  if (source?.page_number) return `${text.page} ${source.page_number}`;
  if (!source?.page_from && !source?.page_to)
    return `${sectionLabel} ${source?.chunk_index ?? "-"}`;
  if (
    source.page_from &&
    source.page_to &&
...
const enText = {
  ...
  processingBody:
    "Chat will be ready when at least one selected PDF reaches Ready status.",
